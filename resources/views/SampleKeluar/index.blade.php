...
@section('content')
    ...
    <div class="nav pt-2 pb-2 mb-0 mt-0">
        ...
        <form class="d-flex justify-content-between align-items-center nav-input-container">
            <div class="nav-input-group">
                <input type="search" class="nav-input" placeholder="Cari Kode, Customer, Expedisi" name="search"
    ...
    <a href="/samplekeluarexport" class="btn btn-outline-success" id="export-sk">Export</a>
    <button class="btn btn-outline-warning" id="show-search">Pencarian By Tanggal</button>
    <div class="col-12">
        <form action="/samplekeluar" method="get" id="pencarian-tanggal">
    ...
    <div class="col-12 mt-2 pb-2 pt-2">
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    @if ($samplekeluar->count())
                        @foreach ($samplekeluar as $sample)
                            <tr>
                                <td>{{ $samplekeluar->firstItem() + $loop->index }}</td>
                                <td>{{ $sample->kode_sample_keluar }}</td>
                                <td>{{ $sample->segment }}</td>
                                <td>{{ date('d-F-Y', strtotime($sample->tanggal_kirim)) }}</td>
                                <td>{{ $sample->expedisi_pengiriman }}</td>
                                ...
                                <td>
                                    <div class="d-flex flex-row justify-content-between align-items-start d-inline">
                                        <a href="/samplekeluar/{{ $sample->id }}" class="btn-statistics">
                                            <img src="{{ asset('template/assets/img/global/eye.svg') }}" alt="">
                                        </a>

                                        <a href="/samplekeluar/{{ $sample->id }}/edit" class="btn-statistics">
                                            <img src="{{ asset('template/assets/img/global/edit.svg') }}" alt="">
                                        </a>

                                        <form action="/samplekeluar/{{ $sample->id }}" method="post" class="d-inline">
                                            @method('delete')
                                            @csrf
    ...
    {{-- PAGINATION START --}}
    {{ $samplekeluar->links('vendor.pagination.bootstrap-5') }}
    {{-- PAGINATION END --}}
